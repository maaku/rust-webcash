...
fn is_webcash_amount(amount: Decimal) -> bool {
    let webcash_min_amount = Decimal::new(1, DECIMAL_POINTS);
    ...
    let valid_precision = ((amount * Decimal::new(i64::pow(10, DECIMAL_POINTS), 0))
        % Decimal::new(1, 0))
        == Decimal::new(0, 0);
    if !valid_precision {
        return false;
...
fn parse_webcash_amount(amount_str: &str) -> Option<Decimal> {
    ...
    let min_length_after_prefix_removal = 1;
    let max_length_after_prefix_removal =
        MAX_WEBCASH.to_string().len() + 1 + (DECIMAL_POINTS as usize);
    if !(amount_str.len() >= min_length_after_prefix_removal
        && amount_str.len() <= max_length_after_prefix_removal)
    {
        return None;
    }
    if !amount_str
